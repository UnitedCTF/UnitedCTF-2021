<?php
// flag-08dfacc89843e423youmasterfileuploadbugs
session_start();

// Run `apt-get install php-mime-type` so mime_content_type works.
...
if (isset($_FILES["upfile"])) {
	...
	if ($_FILES['upfile']['size'] > 131072 || $_FILES['upfile']['error'] === UPLOAD_ERR_INI_SIZE) {
		...
		echo "This file extension is not supported. (1)";
	} else if ($extension !== "png" && $extension !== "jpg" && $extension !== "jpeg" && $extension !== "gif") {
		echo "This file extension is not supported. (2)";
	} else if (mime_content_type($filename) != "image/png" && mime_content_type($filename) != "image/jpg" &&
		       mime_content_type($filename) != "image/jpeg" && mime_content_type($filename) != "image/png") {
		echo "This file extension is not supported. (3)";
